...
class ClassExporter(Exporter):
    ...
    def ScopeName(self):
        return f'{makeid(self.class_.FullName())}_scope'
    ...
    def Write(self, codeunit):
        ...
        code += indent + boost_ns + f'class_< {template_params} >'
    ...
    def ExportBases(self, exported_names):
        ...
        for level in hierarchy:
            ...
            if exported:
                break
        if exported:
            code = f"{namespaces.python}bases< {', '.join(exported)} > "
    ...
    def ExportConstructors(self):
        ...
        def init_code(cons):
            ...
            if max_params:
                if min_params:
                    init += ', '
                init += f'{py_ns}optional< {max_params} >'
            init += ' >()'
            return init
        ...
        if not constructors:
            # declare no_init
            self.Add('constructor', f'{py_ns}no_init')
        else:
    ...
    def ExportVariables(self):
        ...
        for var in vars:
            ...
            def_ = '.def_readonly' if var.type.const else '.def_readwrite'
            code = f'{def_}("{name}", &{fullname})'
            self.Add('inside', code)
    ...
    def GetAddedMethods(self):
        result = []
        if added_methods := self.info.__added__:
    ...
    def ExportMethods(self):
        ...
        def Pointer(m):
            ...
            return f'&{wrapper.FullName()}' if wrapper else m.PointerDeclaration()
        ...
        for method in methods:
            ...
            if policy:
                policy = f', {namespaces.python}{policy.Code()}()'
            # check for overloads
            overload = ''
            if method.minArgs != method.maxArgs:
                ...
                overload = f', {namespaces.pyste}{overload_name}()'

            # build the .def string to export the method
            pointer = Pointer(method)
            code = f'.def("{name}", {pointer}'
            code += policy
            code += overload
            code += ')'
            self.Add('inside', code)
    ...
    def ExportVirtualMethods(self, codeunit):        
        has_virtual_methods = any(
            type(member) == Method and member.virtual for member in self.class_
        )
    ...
    def ExportOperators(self):
        ...
        def GetOperand(param):
            ...
            if param.name == self.class_.FullName():
                return f'{namespaces.python}self'
            else:
                return f'{namespaces.python}other< {param.name} >()'
        ...
        def HandleSpecialOperator(operator):
            ...
            if is_str:
                namespace = f'{namespaces.python}self_ns::'
                self_ = f'{namespaces.python}self'
                return f'.def({namespace}str({self_}))'
        ...
        for operator in operators:
            ...
            if export_as_method:
                ...
                if policy_obj:
                    policy = f', {policy_obj.Code()}()'
                self.Add('inside', f'.def("{rename}", {pointer}{policy})')
                ...
                if is_unary:
                    self.Add('inside', f'.def( {operator.name}{namespaces.python}self )')
                else:
                    # binary operator
                    if len(operator.parameters) == 2:
                        ...
                        left_operand = f'{namespaces.python}self'
                        right_operand = GetOperand(operator.parameters[0])
                    self.Add('inside', f'.def( {left_operand} {operator.name} {right_operand} )')
        ...
        for converter in converters:
            ...
            if info.rename or not special_code:
                ...
                policy_code = ''
                if info.policy:
                    policy_code = f', {info.policy.Code()}()'
                self.Add('inside', f'.def("{name}", {pointer}{policy_code})')
    ...
    def ExportSmartPointer(self):
        if smart_ptr := self.info.smart_ptr:
            class_name = self.class_.FullName()
            smart_ptr = smart_ptr % class_name
            self.Add(
                'scope',
                f'{namespaces.python}register_ptr_to_python< {smart_ptr} >();',
            )
    ...
    def ExportOpaquePointerPolicies(self):
        ...
        for method in methods:
            ...
            if self.info[method.name].policy == return_opaque_policy:
                if macro := exporterutils.EspecializeTypeID(method.result.name):
...
def _ParamsInfo(m, count=None):
    ...
    params = [f'{t} {n}' for t, n in zip(param_types, param_names)]
...
class _VirtualWrapperGenerator(object):
    ...
    def __init__(self, class_, bases, info, codeunit):
        ...
        self.bases = bases[:]
        self.info = info
        self.wrapper_name = f'{makeid(class_.FullName())}_Wrapper'
    ...
    def DefaultImplementationNames(self, method):
        ...
        '''
        base_name = f'default_{method.name}'
    ...
    def Declaration(self, method, indent):
        ...
        if param_names_str:
            param_names_str = f', {param_names_str}'
    ...
    def MethodDefinition(self, method):
        ...
        if method.abstract:
            ...
            default_pointers = [f'&{wrapper_name}::{x}' for x in default_names]
        else:
            default_pointers = []
            # create a list of default-impl pointers
            minArgs = method.minArgs
            maxArgs = method.maxArgs
            constantness = ' const' if method.const else ''
            for impl_name, argNum in zip(default_names, range(minArgs, maxArgs+1)):
                ...
                signature = f'{result} ({wrapper_name}::*)({params}){constantness}'
                default_pointer = f'({signature})&{wrapper_name}::{impl_name}'
                default_pointers.append(default_pointer)

        # get the pointer of the method
        pointer = method.PointerDeclaration()
        if method.abstract:
            pointer = f'{namespaces.python}pure_virtual({pointer})'
        ...
        if default_pointers:
            definitions.append(
                f'.def("{rename}", {pointer}, {default_pointers[-1]}{policy})'
            )
            definitions.extend(
                f'.def("{rename}", {default_pointer}{policy})'
                for default_pointer in default_pointers[:-1]
            )
        else:
            definitions.append(f'.def("{rename}", {pointer}{policy})')
        return definitions
    ...
    def GenerateVirtualMethods(self):
        ...
        def MethodSig(method):
            ...
            result = method.result.FullName() if method.result else ''
            params = ', '.join([x.FullName() for x in method.parameters])
            return f'{result} {method.name}({params}){const}{method.Exceptions()}'  
        ...
        for member in all_methods:
            ...
            if IsVirtual(member) and sig not in already_added:
                self.virtual_methods.append(member)
                already_added[sig] = 0
    ...
    def GenerateVirtualWrapper(self, indent):
        ...
        for cons in self.Constructors(): # only public constructors
            ...
            code += cons_code
        body = [
            self.Declaration(method, indent)
            for method in self.virtual_methods
